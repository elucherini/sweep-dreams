-- Add "nearest" RPC helpers for the puck-based map UI.
--
-- Goals:
-- - schedules_near_closest_block: return only schedules for the single closest block (both sides).
-- - parking_regulation_nearest: return only the single closest parking regulation.

-- 1) Closest sweeping block (both sides)
...
-- 2) Closest parking regulation (single row)
DROP FUNCTION IF EXISTS public.parking_regulation_nearest(double precision, double precision, double precision);

CREATE FUNCTION public.parking_regulation_nearest(
