...
def assert_window(
    window: tuple[datetime, datetime], start: datetime, end: datetime
) -> None:
...
def test_holiday_and_unknown_weekday_raise(schedule_factory):
    ...
    with pytest.raises(ValueError, match="only on holidays"):
        next_sweep_window(
            holiday_schedule, now=datetime(2024, 1, 1, 9, tzinfo=PACIFIC_TZ)
        )

    bad_label_schedule = schedule_factory(week_day="Funday", weeks=(1,))
    with pytest.raises(ValueError, match="Unknown weekday label"):
        next_sweep_window(
            bad_label_schedule, now=datetime(2024, 1, 1, 9, tzinfo=PACIFIC_TZ)
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            datetime(
                2024, 3, 8, 14, tzinfo=PACIFIC_TZ
            ),  # exactly at end -> next occurrence
        ...
        (
            datetime(
                2024, 3, 23, 9, tzinfo=PACIFIC_TZ
            ),  # after 4th Friday -> roll to next month
...
def test_over_midnight_window_is_contiguous(schedule_factory):
    ...
    now = datetime(
        2024, 4, 2, 0, 30, tzinfo=PACIFIC_TZ
    )  # During the overnight window that began Apr 1
...
def test_occurrence_gap_skips_months_without_target_weekday(schedule_factory):
    ...
    now = datetime(
        2025, 2, 1, 12, tzinfo=PACIFIC_TZ
    )  # February 2025 has only four Saturdays
    start = datetime(
        2025, 3, 29, 9, tzinfo=PACIFIC_TZ
    )  # First upcoming month with a 5th Saturday
